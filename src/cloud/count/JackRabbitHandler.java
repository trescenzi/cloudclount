...
public class JackRabbitHandler {
    ...
    private Session session; //the login session
    private Repository repo;//the repo
    private SimpleCredentials credientials;//the credientials
    private GregorianCalendar cal;//a calendar for getting dates
    private String list;//a list of all of the data in the repo 
    private String user;
        ...
        if(session!=null){
            try{
                ...
                while(pi.hasNext()){
                    ...
                    fop.flush();
                    fop.close();
                    JOptionPane.showMessageDialog(null, "Download has finished");
                }
            }catch(Exception e){
    ...
    private void attemptReconnect(){
        try{
           ...
           System.out.println("There has been an error in reconnecting to the "
                   + "server");
       }
    }
